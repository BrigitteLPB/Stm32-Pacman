...
#define OUTPUT_ON_UART		0		// définit si les output sortent sur l'UART 2
#define	PRINTF(code);		if(OUTPUT_ON_UART){printf(code);}	// permet de désactivé les printfs
	...
	if(flag_10ms && initialized)	//le cadencement de cette portion de code à 10ms permet d'éliminer l'effet des rebonds sur le signal en provenance du bouton.
	{
		...
		switch(state)
		{
			...
			case WAIT_RELEASE:
				if(!current_button)
				{
					PRINTF("release button after long press\n");
					state = WAIT_BUTTON;
				}
				break;
			default:
